...
export const ActiverUsers = ({ property }: { property: Property }) => {
  ...
  const { data, refetch, isLoading } = useQuery(["paymentsOvertime"], {
    queryFn: () =>
      axios
        .get(
          `${process.env.NEXT_PUBLIC_API_URL}/backoffice/users/timeframes?dateFrom=${startDate}T00:00:00.000Z&dateTo=${endDate}T00:00:00.000Z&frame=${periodsList[selectedPeriod].value}&token=${property._id}`,
          {
            headers: {
